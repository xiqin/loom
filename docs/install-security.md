...
loom 提供多种安装方式，安全等级不同。本文档说明各方式的安全特性和风险。
...
- 自动检测冲突，冲突文件备份到 `.loom-backup/`
- 写入 `.loom/install-manifest.json` 记录所有文件的 SHA-256 校验和
- 自动更新 `.gitignore` 忽略 `.loom-backup/`
...
- `loom init` 执行相同的冲突检测和备份逻辑
- 写入 manifest 记录校验和

**风险**：

- 需要信任 npm registry 中的 `loom-engineering` 包
- 全局安装的 CLI 工具对所有项目可用
...
3. **loom 管理检测**：检查已有文件是否由 loom 管理（通过版本标记）
...
| -------------------------- | -------------------- | ------------ |
| 文件不存在                 | 直接写入             | 直接写入     |
| 文件由 loom 管理（同版本） | 跳过，提示已安装     | 覆盖         |
| 文件由 loom 管理（旧版本） | 提示用 `loom update` | 覆盖         |
| 文件存在但非 loom 管理     | 报错，列出冲突       | 备份后覆盖   |

## 安装后验证

```bash
loom doctor    # 检查安装状态
loom list      # 列出已安装的 skills 和 commands
```
...
- 冲突文件自动备份到 `.loom-backup/`
- 备份目录名包含时间戳：`.loom-backup/<timestamp>/`
- 自动保留最近 3 份备份，清理更早的
- `.loom-backup/` 自动加入 `.gitignore`

## Manifest 文件

`.loom/install-manifest.json` 记录：

```json
{
  "version": "当前版本号",
  "tool": "claude-code",
  "filesCreated": ["CLAUDE.md", ".loom/skills/..."],
  "filesUpdated": [],
  "backups": [".loom-backup/20260509-120000/"],
  "hooksInstalled": true,
  "fileChecksums": {
    "CLAUDE.md": "sha256-hex...",
    ".loom/skills/brainstorming/SKILL.md": "sha256-hex..."
  },

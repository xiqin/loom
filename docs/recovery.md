...
卸载器读取 `.loom/install-manifest.json`，将文件分为三类：
...
```json
{
  "fileChecksums": {
    "CLAUDE.md": "a1b2c3d4...",
    ".loom/skills/brainstorming/SKILL.md": "e5f6g7h8..."
  }
}
```
...
如果 `.loom/install-manifest.json` 不存在，卸载器拒绝执行：
...
```
1. 读取 .loom/install-manifest.json
...
| 清理项 | 说明 |
|--------|------|
| `.loom-backup/` | 删除所有备份文件 |
| `.gitignore` loom 条目 | 移除 `# loom-engineering` 注释和 `.loom-backup/` 行 |

**注意**：`--purge` 也不能越过 manifest 边界。它只清理 loom 管理的备份和 .gitignore 条目，不会删除用户修改过的文件。
...
重新安装会生成所有文件。之前运行 `/loom-init-project` 生成的文件（constitution.md、MEMORY.md 等）不受影响，因为它们不在 loom 的安装 manifest 中。

### 恢复被 --force 覆盖的文件

安装时使用 `--force` 会将冲突文件备份到 `.loom-backup/`：

```bash
ls .loom-backup/                    # 查看备份目录
ls .loom-backup/20260509-120000/    # 查看具体备份
cp .loom-backup/20260509-120000/CLAUDE.md .  # 恢复文件
```
...
- 安装时检测到冲突文件，自动备份到 `.loom-backup/<timestamp>/`
- 自动保留最近 3 份备份，清理更早的
- `.loom-backup/` 自动加入 `.gitignore`
...
# 恢复特定文件
cp .loom-backup/<timestamp>/CLAUDE.md .
...
A: 在。`constitution.md` 由 `/loom-init-project` 生成，不在 loom 安装 manifest 中，卸载时不会被删除。

### Q: 卸载后 MEMORY.md 还在吗？

A: 在。同上，`MEMORY.md` 由 `/loom-init-project` 生成。

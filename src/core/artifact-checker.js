 ...
 */
export function isReportPassing(specDir, filename, options = {}) {
...
export function validateReportEvidence(specDir, content) {
  ...
  if (evidenceFile) {
    ...
    if (isAbsolute(evidenceFile) || rel.startsWith('..') || isAbsolute(rel)) {
      ...
      actualHash = sha256File(candidate);
...
export function inferStageFromArtifacts(specDir) {
  ...
  if (has('verify-report.md') && markdownReportPasses(specDir, 'verify-report.md')) return 'synced';
  if (has('convergence-report.json') && jsonStatusIn(specDir, 'convergence-report.json', ['converged'])) return 'verification';
  ...
  if (has('artifact-analysis.json') && jsonStatusIn(specDir, 'artifact-analysis.json', ['pass'])) return 'approved';
...
function jsonStatusIn(specDir, file, allowed) {
  try {
...
function markdownReportPasses(specDir, file) {
  try {
 ...
 */
export function validateSkillOutput(specDir, skillName) {
  const errors = [];
  const warnings = [];
    ...
    'loom-subagent-driven-development': {
      ...
      validate(dir) {
        ...
        if (!existsSync(testReportPath)) {
          ...
          const reportContent = readFileSync(testReportPath, 'utf-8');
 ...
 * @returns {{ valid: boolean, errors: string[], warnings: string[] }}
 */
export function validatePipelineConsistency(specDir, completedStages) {
  const errors = [];
  const stageOutputs = {
  ...
  for (const stage of completedStages) {
    ...
    for (const out of outputs) {
      if (!existsSync(join(specDir, out))) {
 ...
 * @returns {{ ok: boolean, missing: string[] }}
 */
export function checkRequiredSections(specDir, filename, requiredSections) {
  const path = join(specDir, filename);
  if (!existsSync(path)) return { ok: false, missing: [filename] };
  const content = readFileSync(path, 'utf-8');
  const missing = [];

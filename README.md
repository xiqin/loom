...
智能模式经过三段决策：规则短路（0 token）→ AI fallback（可选）→ 规则兜底。执行前会向用户说明选择来源、风险等级和步骤顺序；初始化后把 `dynamic_steps` 写入 `pipeline.state.json`，并在自动生成的 `progress.md` 中记录当前阶段和动态步骤，便于 AI 在没有对话上下文时继续执行。

...
| `verification` | Step 5 | 完成前验证（前置产出核验、编译验证、占位符扫描、类型一致性、最终一致性核验） |
...
| 5 | verification | 完成前验证（前置产出核验、编译验证、占位符扫描、类型一致性、最终一致性核验） | 验证报告 |
...
| verification | test-report.md 存在且结论为 PASS + reviewer 报告存在且判定通过 |

...
description: >
  ...
  Output pipeline-plan.md for user review before initialization.
...
### Step 6：输出 pipeline-plan.md

在 specDir 写 `pipeline-plan.md`：
...
输出 `pipeline-plan.md` 后提示用户：

- `loom run --spec-dir <dir> --approve-pipeline` 确认并初始化
- 或手动调整 `pipeline-plan.md` 后再确认

用户确认后才写入 `pipeline.state.json` 的 `dynamic_steps` 字段。

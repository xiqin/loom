...
- 必须遵循实现→重构→测试循环（实现先行，重构强制，测试验证真实 public API）
- 先实现 task behavior，再重构代码结构，最后针对实现的 public boundary 写行为验证测试
- 测试失败等同 BLOCKER
- 写测试前必须审查新增参数、接口、抽象和扩展点：每个设计都要有独立的业务理由；通用策略、插件或业务抽象必须能指出至少两个真实生产场景，否则删除或延后。外部网络/计费 API、不可回滚副作用、不可控非确定性这三种允许注入边界可以只有一个生产实现，但必须记录替换边界和业务理由
...
- 重构是强制步骤：实现完成后必须先重构（消除重复、改善命名、理清职责），再写测试——测试会锁定它接触的结构，凌乱实现会被测试锁死
- 覆盖率是发现工具而非门槛：测试通过后查看未覆盖区域，逐项判断是否有独立可错路径，有则补测，无则记录依据
- 每个测试只验证一个行为
...
5. **记录测试前设计审查结果**：若新增参数、接口、抽象或扩展点，必须说明真实业务场景、生产调用方或真实实现，以及不能使用具体实现的理由；若没有新增生产抽象，明确记录“无测试专用生产设计”。仅为测试隔离引入的设计不能通过补充 rationale 合法化，必须删除或延后。
6. **生成 handoff 文件** `specs/<date+feature>/handoffs/<task-id>.json`，格式：
   ```json
   {
     "task_id": "TN",
     "status": "done",

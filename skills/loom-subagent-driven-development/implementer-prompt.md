...
3. 编写对应的行为验证测试文件（行为级/集成测试优先），必须持久化到项目代码库的标准测试目录（如 `tests/`、`__tests__/`、`src/**/__tests__/` 等项目约定目录），不得作为临时验证后删除
...
- 必须遵循红-绿-重构循环（红绿对象为**行为级测试**，非单元测试）
- 先写失败测试，运行确认失败（"红"是硬要求，证明测试非伪测试），再写实现代码
- 测试失败等同 BLOCKER
- 测试层级：**行为级/集成测试优先，真实依赖**——文件 I/O 用 `mkdtempSync` 真实临时目录 + afterEach 清理，单元测试仅作为行为级触达不到的纯逻辑分支的补充
- 仅三种边界允许注入替换，且用内联 fake 对象、不为测试建实现类：a) 外部网络/计费 API  b) 不可回滚副作用（邮件/支付） c) 不可控非确定性（如 crypto.random）
- **禁止为测试目的给生产代码添加构造参数/接口/仅测试方法**（test-induced design damage）；若唯一动机是"测试时不碰磁盘"，用 mkdtempSync
- 每个测试只验证一个行为
- 每个 `behavior_ids` 至少对应一个持久化行为验证测试（持久化到项目标准测试目录，不得临时验证后删除），并写入 `traceability.json` 的 behavior 级 `tests`

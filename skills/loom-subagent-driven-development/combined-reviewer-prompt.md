...
拦截为可测试性而引入的防御性抽象（test-induced design damage）。对 diff 中每个新增构造参数、接口/抽象层、仅测试使用的方法，逐项检查：

1. **新增构造参数**（如 `constructor(x, { fs, db } = {})`）是否有 ≥1 个非默认值的生产调用方？（测试中的 mock 传入不算）
2. **新增抽象/接口**是否有 ≥2 个真实实现？（内存 mock 实现不算"真实实现"）
...
命中任一项且 diff 或 handoff 中无 rationale → **阻断**，修复指令要求删除该抽象或举证其必要性。
